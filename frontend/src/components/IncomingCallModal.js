...
const IncomingCallModal = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center">
      <div className={`bg-gradient-to-br from-pink-500 to-purple-600 rounded-3xl p-8 max-w-sm w-full mx-4 shadow-2xl transition-transform ${isRinging ? 'scale-105' : 'scale-100'}`}>
        {/* Caller Info */}
        ...
        <div className="flex items-center justify-center gap-6">
          ...
          <button
            onClick={onAccept}
            className={`p-6 bg-green-500 rounded-full hover:bg-green-600 transition transform hover:scale-110 shadow-lg ${isRinging ? 'animate-bounce' : ''}`}
          >
